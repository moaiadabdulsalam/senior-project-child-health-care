...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      this.logger.log(`Chat socket connected | socketId=${client.id} | userId=${payload.userId}`);
  ...
  emitNewMessage(conversationId: string, payload: unknown) {
    this.server.to(`conversation:${conversationId}`).emit('chat:new-message', payload);
  }
  ...
  emitConversationRead(conversationId: string, payload: unknown) {
    this.server.to(`conversation:${conversationId}`).emit('chat:conversation-read', payload);
  }
